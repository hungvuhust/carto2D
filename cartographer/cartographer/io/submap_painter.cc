...
void CairoPaintSubmapSlices(
  ...
  for (auto &pair: submaps) {
    ...
    cairo_matrix_init(
        &matrix, homo(1, 0), homo(0, 0), -homo(1, 1), -homo(0, 1), homo(0, 3),
        -homo(1, 3));
...
PaintSubmapSlicesResult PaintSubmapSlices(
  ...
  const Eigen::Array2f origin(
      -bounding_box.min().x() + kPaddingPixel,
...
void FillSubmapSlice(
  ...
  ::cartographer::mapping::Submap2D submap(
      proto.submap_2d(), conversion_tables);
  ...
  submap_slice->surface= DrawTexture(
      pixels.intensity, pixels.alpha, texture_proto.width(),
...
void DeserializeAndFillSubmapSlices(
  ...
  for (const auto &trajectory: deserializer->pose_graph().trajectory()) {
    for (const auto &submap: trajectory.submap()) {
      submap_poses[mapping::SubmapId(
          trajectory.trajectory_id(), submap.submap_index())]=
  ...
  while (deserializer->ReadNextSerializedData(&proto)) {
    if (proto.has_submap() && Has2DGrid(proto.submap())) {
      ...
      FillSubmapSlice(
          submap_poses.at(id), submap, &(*submap_slices)[id],
          conversion_tables);
    }
  }
}
...
SubmapTexture::Pixels UnpackTextureData(
    const std::string &compressed_cells, const int width, const int height) {
...
UniqueCairoSurfacePtr DrawTexture(
  ...
  for (size_t i= 0; i < intensity.size(); ++i) {
    ...
    cairo_data->push_back(
        (alpha_value << 24) | (intensity_value << 16) | (observed << 8) | 0);
  }
