...
// 'output_file'.
void WriteBinaryPlyHeader(
    const bool has_color, const bool has_intensities,
    const std::vector<std::string> &comments, const int64 num_points,
...
void WriteBinaryPlyPointCoordinate(
...
void WriteBinaryIntensity(
    const float intensity, FileWriter *const file_writer) {
  // TODO(sirver): This ignores endianness.
  CHECK(file_writer->Write(
      reinterpret_cast<const char *>(&intensity), sizeof(float)));
}

void WriteBinaryPlyPointColor(
...
PlyWritingPointsProcessor::PlyWritingPointsProcessor(
    ...
    : next_(next), comments_(comments), num_points_(0), has_colors_(false),
      file_(std::move(file_writer)) {}

PointsProcessor::FlushResult PlyWritingPointsProcessor::Flush() {
  WriteBinaryPlyHeader(
      has_colors_, has_intensities_, comments_, num_points_, file_.get());
...
void PlyWritingPointsProcessor::Process(std::unique_ptr<PointsBatch> batch) {
  ...
  if (num_points_ == 0) {
    ...
    WriteBinaryPlyHeader(
        has_colors_, has_intensities_, comments_, 0, file_.get());
  }
  if (has_colors_) {
