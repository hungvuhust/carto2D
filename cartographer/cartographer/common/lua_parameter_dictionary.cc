...
template <typename T> void GetValueFromLuaTable(lua_State *L, const T &key) {
  PushValue(L, key);
  lua_rawget(L, -2);
}
...
template <typename T> bool HasKeyOfType(lua_State *L, const T &key) {
...
LuaParameterDictionary::LuaParameterDictionary(
    ...
    : L_(lua_newthread(L)), file_resolver_(std::move(file_resolver)),
      reference_count_(reference_count) {
  CHECK_NOTNULL(L_);
...
std::unique_ptr<LuaParameterDictionary>
    LuaParameterDictionary::GetDictionary(const std::string &key) {
  CheckHasKeyAndReference(key);
  GetValueFromLuaTable(L_, key);
...
std::string
    LuaParameterDictionary::DoToString(const std::string &indent) const {
  std::string result             = "{";
  ...
  const auto top_of_stack_to_string= [this, indent,
    ...
    switch (value_type) {
    case LUA_TBOOLEAN: return PopBool() ? "true" : "false"; break;
    case LUA_TSTRING: return PopString(Quoted::YES); break;
    case LUA_TNUMBER: {
    ...
    default: LOG(FATAL) << "Unhandled type " << lua_typename(L_, value_type);
    }
  };
...
int LuaParameterDictionary::LuaInclude(lua_State *L) {
  ...
  if (std::find(
          parameter_dictionary->included_files_.begin(),
