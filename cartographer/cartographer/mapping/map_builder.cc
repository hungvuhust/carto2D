...
void MapBuilder::SerializeState(
  ...
  io::WritePbStream(
      *pose_graph_, all_trajectory_builder_options_, writer,
      include_unfinished_submaps);
}

bool MapBuilder::SerializeStateToFile(
...
std::map<int, int> MapBuilder::LoadState(
  ...
  for (const proto::Trajectory &trajectory_proto:
    ...
    for (const proto::Trajectory::Submap &submap_proto:
      ...
      submap_poses.Insert(
          SubmapId{
              trajectory_proto.trajectory_id(), submap_proto.submap_index()},
  ...
  while (deserializer.ReadNextSerializedData(&proto)) {
    ...
    case SerializedData::kSubmap: {
      ...
      pose_graph_->AddSubmapFromProto(
          submap_poses.at(submap_id), proto.submap());
      break;
    }
