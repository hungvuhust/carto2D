...
Submap2D::Submap2D(
    const proto::Submap2D &proto, ValueConversionTables *conversion_tables)
  ...
  if (proto.submap_2d().has_grid()) {
    if (proto.submap_2d().grid().has_probability_grid_2d()) {
      grid_= absl::make_unique<ProbabilityGrid>(
          proto.submap_2d().grid(), conversion_tables_);
...
std::unique_ptr<RangeDataInserterInterface>
  ...
  default: LOG(FATAL) << "Unknown RangeDataInserterType.";
  }
}

std::unique_ptr<GridInterface>
    ActiveSubmaps2D::CreateGrid(const Eigen::Vector2f &origin) {
  ...
  case proto::GridOptions2D::TSDF:
    return absl::make_unique<TSDF2D>(
        ...
        options_.range_data_inserter_options()
            ...
            .maximum_weight(),
        &conversion_tables_);
  default: LOG(FATAL) << "Unknown GridType.";
  }
}
