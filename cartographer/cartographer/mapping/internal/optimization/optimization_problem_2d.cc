    ...
    return absl::make_unique<transform::Rigid3d>(
        Interpolate(
            transform::TimestampedTransform{
                prev_it->time, prev_it->pose.value()},
        ...
        const transform::Rigid3d starting_point=
            landmark_node.second.global_landmark_pose.has_value()
                ...
                : GetInitialLandmarkPose(
                      observation, prev->data, next->data, *prev_node_pose,
                      *next_node_pose);
...
void OptimizationProblem2D::AddImuData(
...
void OptimizationProblem2D::AddTrajectoryNode(
    const int trajectory_id, const NodeSpec2D &node_data) {
...
void OptimizationProblem2D::InsertTrajectoryNode(
    const NodeId &node_id, const NodeSpec2D &node_data) {
...
void OptimizationProblem2D::Solve(
  ...
  for (const auto &submap_id_data: submap_data_) {
    ...
    C_submaps.Insert(
        submap_id_data.id, FromPose(submap_id_data.data.global_pose));
  ...
  AddLandmarkCostFunctions(
      landmark_nodes, node_data_, &C_nodes, &C_landmarks, &problem,
    ...
    for (; node_it != trajectory_end; ++node_it) {
      ...
      if (!fixed_frame_pose_initialized) {
        ...
        C_fixed_frames.emplace(
            trajectory_id, FromPose(fixed_frame_pose_in_map));
