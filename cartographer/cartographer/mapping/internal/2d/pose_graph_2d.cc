...
PoseGraph2D::PoseGraph2D(
    ...
    : options_(options), optimization_problem_(std::move(optimization_problem)),
...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  CHECK(
      data_.submap_data.at(last_submap_id).submap == insertion_submaps.back());
  const SubmapId front_submap_id{
      trajectory_id, last_submap_id.submap_index - 1};
  CHECK(
      data_.submap_data.at(front_submap_id).submap ==
      insertion_submaps.front());
...
NodeId PoseGraph2D::AddNode(
  ...
  const NodeId node_id= AppendNode(
      constant_data, trajectory_id, insertion_submaps, optimized_pose);
  ...
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    return ComputeConstraintsForNode(
        node_id, insertion_submaps, newly_finished_submap);
  });
  return node_id;
}
...
void PoseGraph2D::AddImuData(
    const int trajectory_id, const sensor::ImuData &imu_data) {
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph2D::AddOdometryData(
  ...
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    ...
    if (CanAddWorkItemModifying(trajectory_id)) {
      optimization_problem_->AddFixedFramePoseData(
          trajectory_id, fixed_frame_pose_data);
    }
    return WorkItem::Result::kDoNotRunOptimization;
  });
}

void PoseGraph2D::AddLandmarkData(
    int trajectory_id, const sensor::LandmarkData &landmark_data) {
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    absl::MutexLock locker(&mutex_);
    if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph2D::ComputeConstraint(
...
common::Time PoseGraph2D::GetLatestNodeTime(
...
void PoseGraph2D::HandleWorkQueue(
    const constraints::ConstraintBuilder2D::Result &result) {
  {
    ...
    data_.constraints.insert(
        data_.constraints.end(), result.begin(), result.end());
  }
  RunOptimization();
...
void PoseGraph2D::WaitForAllComputations() {
  ...
  while (!mutex_.AwaitWithTimeout(
      absl::Condition(&predicate), absl::FromChrono(common::FromSeconds(1.)))) {
    report_progress();
  }
...
void PoseGraph2D::AddSubmapFromProto(
  ...
  const SubmapId submap_id= {
      submap.submap_id().trajectory_id(), submap.submap_id().submap_index()};
      ...
      transform::Project2D(global_submap_pose);
  {
    ...
    const std::shared_ptr<const Submap2D> submap_ptr=
        std::make_shared<const Submap2D>(
            submap.submap_2d(), &conversion_tables_);
...
void PoseGraph2D::AddNodeFromProto(
    const transform::Rigid3d &global_pose, const proto::Node &node) {
  const NodeId node_id= {
      node.node_id().trajectory_id(), node.node_id().node_index()};
  std::shared_ptr<const TrajectoryNode::Data> constant_data=
...
void PoseGraph2D::AddNodeToSubmap(
    const NodeId &node_id, const SubmapId &submap_id) {
...
void PoseGraph2D::AddSerializedConstraints(
  ...
  AddWorkItem([this, constraints]() LOCKS_EXCLUDED(mutex_) {
    ...
    for (const auto &constraint: constraints) {
      ...
      CHECK(
          data_.trajectory_nodes.at(constraint.node_id).constant_data !=
          nullptr);
...
void PoseGraph2D::RunOptimization() {
  ...
  optimization_problem_->Solve(
      data_.constraints, GetTrajectoryStates(), data_.landmark_nodes);
  absl::MutexLock locker(&mutex_);
...
MapById<NodeId, TrajectoryNodePose>
    PoseGraph2D::GetTrajectoryNodePoses() const {
...
std::map<std::string, transform::Rigid3d>
    PoseGraph2D::GetLandmarkPoses() const {
...
void PoseGraph2D::SetLandmarkPose(
...
void PoseGraph2D::SetInitialTrajectoryPose(
    const int from_trajectory_id, const int to_trajectory_id,
...
transform::Rigid3d
    PoseGraph2D::GetLocalToGlobalTransform(const int trajectory_id) const {
  absl::MutexLock locker(&mutex_);
...
PoseGraphInterface::SubmapData
    PoseGraph2D::GetSubmapData(const SubmapId &submap_id) const {
  absl::MutexLock locker(&mutex_);
...
PoseGraphInterface::SubmapData
    PoseGraph2D::GetSubmapDataUnderLock(const SubmapId &submap_id) const {
  ...
  return {
      submap, ComputeLocalToGlobalTransform(
                  data_.global_submap_poses_2d, submap_id.trajectory_id) *
                  submap->local_pose()};
}
  ...
  for (const auto &submap_id_data: parent_->data_.submap_data) {
    ...
    submaps.Insert(
        ...
        SubmapData{
            ...
            transform::Embed3D(
                parent_->data_.global_submap_poses_2d.at(submap_id_data.id)
                    .global_pose)});
  }
  return submaps;
}
...
std::vector<SubmapId>
    PoseGraph2D::TrimmingHandle::GetSubmapIds(int trajectory_id) const {
  std::vector<SubmapId> submap_ids;
  const auto &submap_data= parent_->optimization_problem_->submap_data();
...
void PoseGraph2D::TrimmingHandle::SetTrajectoryState(
    int trajectory_id, TrajectoryState state) {
...
void PoseGraph2D::TrimmingHandle::TrimSubmap(const SubmapId &submap_id) {
  ...
  for (const auto &submap_data: parent_->data_.submap_data) {
    if (submap_data.id != submap_id) {
      nodes_to_retain.insert(
          submap_data.data.node_ids.begin(), submap_data.data.node_ids.end());
    }
  }
...
MapById<SubmapId, PoseGraphInterface::SubmapData>
  ...
  for (const auto &submap_id_data: data_.submap_data) {
    submaps.Insert(
        submap_id_data.id, GetSubmapDataUnderLock(submap_id_data.id));
  }
  return submaps;
}
...
void PoseGraph2D::RegisterMetrics(metrics::FamilyFactory *family_factory) {
  ...
  auto *queue_size     = family_factory->NewGaugeFamily(
