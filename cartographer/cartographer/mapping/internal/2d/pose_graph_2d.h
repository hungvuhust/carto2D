...
public:
  ...
  void AddLandmarkData(
      int trajectory_id, const sensor::LandmarkData &landmark_data) override
      LOCKS_EXCLUDED(mutex_);
  ...
  void AddNodeFromProto(
      const transform::Rigid3d &global_pose, const proto::Node &node) override;
  ...
  void                    SetInitialTrajectoryPose(
                         ...
                         const transform::Rigid3d &pose, const common::Time time) override
      LOCKS_EXCLUDED(mutex_);
...
private:
  ...
  common::Time
      GetLatestNodeTime(const NodeId &node_id, const SubmapId &submap_id) const
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);
