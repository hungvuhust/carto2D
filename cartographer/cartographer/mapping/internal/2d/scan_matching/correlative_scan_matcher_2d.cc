...
SearchParameters::SearchParameters(
  ...
  angular_perturbation_step_size=
      kSafetyMargin *
      std::acos(
          1. - common::Pow2(resolution) / (2. * common::Pow2(max_scan_range)));
...
SearchParameters::SearchParameters(
    const int num_linear_perturbations, const int num_angular_perturbations,
...
void SearchParameters::ShrinkToFit(
    const std::vector<DiscreteScan2D> &scans, const CellLimits &cell_limits) {
  ...
  for (int i= 0; i != num_scans; ++i) {
    ...
    for (const Eigen::Array2i &xy_index: scans[i]) {
      ...
      max_bound= max_bound.max(
          Eigen::Array2i(
              cell_limits.num_x_cells - 1, cell_limits.num_y_cells - 1) -
          xy_index);
    }
