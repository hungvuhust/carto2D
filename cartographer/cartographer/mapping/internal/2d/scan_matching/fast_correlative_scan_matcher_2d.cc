...
PrecomputationGrid2D::PrecomputationGrid2D(
    ...
    : offset_(-width + 1, -width + 1),
      wide_limits_(
          limits.num_x_cells + width - 1, limits.num_y_cells + width - 1),
...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  ...
  for (int i= 0; i != options.branch_and_bound_depth(); ++i) {
    const int width= 1 << i;
    precomputation_grids_.emplace_back(
        grid, limits, width, &reusable_intermediate_grid);
  }
}
...
bool FastCorrelativeScanMatcher2D::Match(
  ...
  return MatchWithSearchParameters(
      search_parameters, initial_pose_estimate, point_cloud, min_score, score,
      pose_estimate);
}
...
bool FastCorrelativeScanMatcher2D::MatchFullSubmap(
  ...
  return MatchWithSearchParameters(
      search_parameters, center, point_cloud, min_score, score, pose_estimate);
}
...
void FastCorrelativeScanMatcher2D::ScoreCandidates(
  ...
  std::sort(
      candidates->begin(), candidates->end(), std::greater<Candidate2D>());
}
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  for (const Candidate2D &candidate: candidates) {
    ...
    best_high_resolution_candidate= std::max(
        ...
        BranchAndBound(
            discrete_scans, search_parameters, higher_resolution_candidates,
