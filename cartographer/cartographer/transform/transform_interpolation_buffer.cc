...
TransformInterpolationBuffer::TransformInterpolationBuffer(
  ...
  for (const mapping::proto::Trajectory::Node &node: trajectory.node()) {
    Push(
        common::FromUniversal(node.timestamp()),
        transform::ToRigid3(node.pose()));
  }
}

void TransformInterpolationBuffer::Push(
    const common::Time time, const transform::Rigid3d &transform) {
...
transform::Rigid3d
    TransformInterpolationBuffer::Lookup(const common::Time time) const {
