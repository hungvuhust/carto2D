...
PointCloud
    FilterByMaxRange(const PointCloud &point_cloud, const float max_range) {
...
VoxelKeyType
    GetVoxelCellIndex(const Eigen::Vector3f &point, const float resolution) {
...
template <class T, class PointFunction>
std::vector<T> RandomizedVoxelFilter(
    const std::vector<T> &point_cloud, const float resolution,
    PointFunction &&point_function) {
...
PointCloud VoxelFilter(const PointCloud &point_cloud, const float resolution) {
  ...
  return PointCloud(
      std::move(filtered_points), std::move(filtered_intensities));
}
